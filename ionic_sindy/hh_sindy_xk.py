...
Library:     GeneralizedLibrary: V^k*xK and V^k*(1-xK), k=0..degree
             (from sindy_tools.make_library)
Optimizer:   ps.STLSQ or ps.SR3 (from sindy_tools.fit_sindy)
"""
import os, warnings
import numpy as np
...
os.makedirs("outputs_hh_xk", exist_ok=True)
...
if __name__ == "__main__":
    ...
    DT         = 0.05
    DEGREES    = [2, 3, 4]
    OPT_NAME   = "SR3"   #STLSQ, SR3, LASSO
    THRESHOLDS = [1e-3, 5e-3, 1e-2, 5e-2, 0.1, 0.3, 0.5, 0.75, 1.0, 2.0]

    print("HH SINDy xK = n^4 | GeneralizedLibrary\n")
    ...
    for degree in DEGREES:
        ...
        for th in THRESHOLDS:
            try:
                ...
                print(f"  {degree} | {th:7.0e} | {n_nz:3} | {r2:.5f} | "
                      f"{rmse_v:.4f} | {n_o}/{n_i}")

                if np.isfinite(rmse_v) and rmse_v < best_rmse:
                    best_rmse = rmse_v
                    best = dict(degree=degree, th=th, model=model, lib=lib_fit,
                                r2=r2, n_nz=n_nz,
                                t_id=t_id, V_id=V_id, xK_id=xK_id)
                results.append(dict(degree=degree, th=th, model=model, lib=lib_fit,
    ...
    for degree in DEGREES:
        ...
        plt.suptitle(f"HH SINDy xK — degree={degree}")
        plt.tight_layout(); plt.subplots_adjust(right=0.78)
        fname = f"outputs_hh_xk/hh_sindy_xk_deg{degree}.png"
        ...
        for r in dr:
            ...
            plt.suptitle(
                f"xK  deg={r['degree']}  th={r['th']:.0e}"
                f"  n={r['n_nz']}  R2={r['r2']:.4f}  RMSE={r['rmse_v']:.2f}mV",
                fontsize=8)
            plt.tight_layout()
            fname_r = (f"outputs_hh_xk/"
                       f"hh_sindy_xk_deg{r['degree']}_th{r['th']:.0e}.png")
...
# --- Exporta métricas (R2, RMSE) vs threshold para .txt ---
    ...
    metrics_dir = "outputs_hh_xk/metrics_txt"
